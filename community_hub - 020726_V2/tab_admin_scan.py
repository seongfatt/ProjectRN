...
def show_admin_scan(selected_date):
    ...
    attended_ids = set()
    try:
        # We only fetch 'participant_id' to save bandwidth and make it ultra-fast
        att_data = supabase.table('attendance').select('participant_id') \
            .eq('date', str(selected_date)) \
            .eq('source', activity) \
            .execute().data
        attended_ids = {rec['participant_id'] for rec in att_data}
    except Exception:
        attended_ids = set()
    ...
    for i, p in enumerate(display_filtered):
        with cols[i % 2]:
            with st.container():
                ...
                # 🚀 SPEED FIX: Check local set instead of querying DB
