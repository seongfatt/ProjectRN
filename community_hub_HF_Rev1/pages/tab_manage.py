def show_manage(selected_date):
    ...
    with tab1:
        if st.session_state.user_role == 'admin':
            with st.expander("Register New Participant"):
                # 🔥 No st.form → instant checkbox reactivity for block consent
    ...
    # ── TAB 2: Activities (🔥 DYNAMIC SESSIONS + FIXED BUTTONS) ──
    with tab2:
        MAX_SESSIONS = 4
        ...
        if not acts:
            ...
            for a in acts:
                ...
                with c4:
                    if st.session_state.user_role == 'admin':
                        # 🔥 FIX: Restore only shows for inactive; 🗑️ only for active
                        if not a.get('active'):
                            ...
                            if st.button("🗑️", key=f"act_del_{act_id}"):
                                ...
                                refresh_data()
                                st.rerun()
                        # 🔥 FIX: Edit now OPENS the editor (no longer deactivates!)
    ...
    # ── TAB 4: Finance & Payment Status (🔥 BLOCK-AWARE) ─────
    with tab4:
        ...
        st.divider()
        # ── B. Payment Dashboard (🔥 BLOCK FILTER) ──
                ...
                with c2:
                    if st.button(btn_label, key=f"pay_dash_{plot_block}_{plot_num}", type=btn_type, use_container_width=True):
                        try:
                            ...
                            # 🔥 CRITICAL: scope update to the plot's OWN block (composite key)
