...
def show_meeting(selected_date):
    ...
    if all_att:
        ...
        # Daily trend (🔥 FIXED to include S3/S4 dynamically)
    ...
    if acts:
        ...
        for i, act in enumerate(acts):
            with act_cols[i % 4]:
                ...
                # 🔥 Show all configured sessions dynamically
                sessions_html = ""
    ...
    if st.button("📊 Generate Excel Report", type="primary", use_container_width=True):
        with st.spinner("Generating Excel report..."):
            # 🔥 FIXED: Pass s3_total and s4_total to the generator
...
# 🔥 FIXED: Added s3_total and s4_total to function signature
def generate_excel_report(start_date, end_date, occupied, available, occupancy_rate, type_rows, all_att, total_records, unique_p, s1_total, s2_total, s3_total, s4_total, act_summary, parts, parts_df, new_c, reg_c, unsigned):
    ...
    if all_att:
        ...
        if act_summary:
            ...
            # 🔥 FIXED: Added S3 and S4 headers and columns
