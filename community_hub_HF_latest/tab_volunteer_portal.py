...
def show_volunteer_portal(token, activity_param=None):
    ...
    if method == "📸 Scan QR Code (Camera)":
        ...
        # Get the list of people who have attended this specific activity before
        try:
        ...
        # Sort: Put people who attend this activity at the top
        fresh_participants.sort(key=lambda p: p['id'] in activity_attendees, reverse=True)
        ...
        if browse_mode == "🔎 Search by Name or ID":
            ...
            if name_search:
                ...
                filtered_participants = fresh_participants[:20]  # Show top 20 if empty
        else:
            # 📋 Show All (Filtered by Activity)
            st.caption(f"Showing all active residents for '{selected_activity}'.")
            filtered_participants = fresh_participants  # Show ALL active residents
