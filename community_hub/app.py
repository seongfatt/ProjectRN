...
import hashlib, os, io, base64, qrcode
from PIL import Image
from config import (supabase, DB_CONNECTED, MOBILE_CSS, ADMIN_PASSWORD, 
    CHECKER_PASSWORD, APP_URL, load_activities, PLOT_TYPES, TOTAL_PLOTS, TYPE_MAP)
...
for k in ['is_authenticated','user_role','show_login','participants','plots','activities','today_date','selected_plot','selected_activity']:
    if k not in st.session_state:
        st.session_state[k] = None if k in ['user_role','selected_plot','selected_activity'] else (False if k=='is_authenticated' else ([] if k in ['participants','plots','activities'] else datetime.now().date()))
