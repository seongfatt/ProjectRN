                ...
                with c4:
                    if st.button(btn_label, key=f"pay_btn_{plot['plot_number']}", type=btn_type, use_container_width=True):
                        try:
                            ...
                            supabase.table('garden_plots').update({'paid': new_status, 'updated_at': datetime.now().isoformat()}).eq('plot_number', plot['plot_number']).execute()
                            log_action(st.session_state.user_role, "MARK_PAID" if new_status else "MARK_UNPAID", f"Plot {plot['plot_number']} - {owner_name}", str(plot['plot_number']))
