...
def show_volunteer_portal(token, activity_param=None):
    ...
    function startScanner() {
        ...
        Html5Qrcode.getCameras().then(cameras => {
            if (cameras && cameras.length) {
                ...
                .then(() => { isScanning = true; }).catch(err => { console.error("Unable to start scanning", err); });
            }
        }).catch(err => { console.error("Unable to get cameras", err); });
    }
    function stopScanner() {
        if (html5QrCode && isScanning) {
            html5QrCode.stop().then(() => { isScanning = false; }).catch(err => { console.error("Unable to stop scanning", err); });
        }
    }
    ...
    method = st.radio("How would you like to check in residents?", ["📱 Scan QR Code (Manual ID)", "⌨️ Phone Number Search", "📝 Register New Resident"], horizontal=False, key="portal_method")
    ...
    if method == "📱 Scan QR Code (Camera)":
        ...
        <div class="method-card" style="color: #1a1a1a !important;">
            ...
            <p style="color: #333333 !important; font-size: 15px; font-weight: 500;">Enter the ID from the resident's QR code card for instant check-in.</p>
        </div>
        """, unsafe_allow_html=True)
        st.markdown('<div id="reader"></div>', unsafe_allow_html=True)
        col1, col2 = st.columns(2)
        with col1:
            if st.button("▶️ Start Camera", use_container_width=True): st.markdown("<script>startScanner();</script>", unsafe_allow_html=True)
        with col2:
            if st.button("⏹️ Stop Camera", use_container_width=True): st.markdown("<script>stopScanner();</script>", unsafe_allow_html=True)
        st.divider()
