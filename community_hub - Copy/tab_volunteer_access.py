...
def show_volunteer_access():
    ...
    if st.session_state.get('new_volunteer_link'):
        ...
        with c1:
            ...
            st.caption("Volunteer QR — scan to open")
        with c2:
            st.markdown(f"""
            <div style="background: #e8f5e9; border-left: 4px solid #4caf50; padding: 15px; border-radius: 4px; color: #1a1a1a;">
                ...
                • Status: 🟢 Active<br>
                • Share via WhatsApp, Telegram, or print the QR
            </div>
            """, unsafe_allow_html=True)

            # WhatsApp share button
            wa_msg = urllib.parse.quote(f"Hi! Here's your volunteer access link for Woodlands Zone 6 Community Hub. Please use it to help with check-ins and registration. Link expires {st.session_state.new_volunteer_expires.strftime('%d %b %Y, %I:%M %p')}: {st.session_state.new_volunteer_link}")
    ...
    if not active:
        ...
        for tok in active:
            ...
            with st.expander(f"🔗 {tok['token'][:16]}... | Used {tok.get('usage_count', 0)}x | Expires: {exp.strftime('%d %b %I:%M %p')}"):
                ...
                with c1:
                    link = f"{APP_URL}/?mode=volunteer&tk={tok['token']}"
