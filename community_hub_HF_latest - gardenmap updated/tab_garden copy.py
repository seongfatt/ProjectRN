    ...
    if st.session_state.user_role == 'admin':
        ...
        with tab_layout:
            ...
            with col_left:
                ...
                # 🔥 SIMPLIFIED DROPDOWN: Shows ALL plots 1-76, including missing ones
                all_plot_options = {}
                            ...
                            with cols_ui[c]:
                                ...
                                if plot_num is None:
                                    if st.button("+", key=f"add_{r}_{c}_{selected_block}", use_container_width=True):
                                        ...
                                        st.session_state[f"add_type_{r}_{c}"] = True
                                        st.rerun()
                                    
                                    if st.session_state.get(f"add_type_{r}_{c}"):
                                        st.markdown("**Size:**")
                                        size_sel = st.radio(
                                            "Size",
                                            ["A", "B", "C", "D"],
                                            horizontal=True,
                                            key=f"type_sel_{r}_{c}",
                                            index=None
                                        )
                                        if size_sel:
                                            if save_garden_layout(selected_block, selected_new_plot, r, c, size_sel):
                                                st.session_state[f"add_type_{r}_{c}"] = False
                                                st.rerun()
                                else:
                                    layout_item = next((item for item in layout_data if item['plot_number'] == plot_num), None)
                                    plot_type = layout_item['plot_type'] if layout_item and 'plot_type' in layout_item else 'B'
