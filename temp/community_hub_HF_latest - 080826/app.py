...
if params.get("mode") == "auto":
    ...
    # 🔥 NEW LOGIC: Load the current activity to get its ACTIVE session IDs
    ...
    if not session_records:
        st.warning(f"No active sessions found for {act} on {formatted_date}.")
        st.stop()

    try:
                # 🔥 1. Record attendance in the master attendance table (for reporting)
        ...
        if existing_att.data and len(existing_att.data) > 0:
            # Update existing attendance record with new session flags
            ...
            updates = {
                ...
                "source": act,  # Keep the last one for legacy fallback
                "activities": current_activities  # Store ALL activities
            }
        ...
        if 'duplicate key' in error_msg or '23505' in error_msg:
            st.info("ℹ️ You are already checked in for this session!")
            st.stop()
        else:
...
st.divider()
st.caption("Woodlands Zone 6 Community Hub | PDPA Compliant | v3.0")