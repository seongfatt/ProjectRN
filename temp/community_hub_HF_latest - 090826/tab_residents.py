...
def show_residents():
    ...
    st.caption(
        "Occupied = Dimmed + Red X | Available = Bright | Green border = Available, Red border = Occupied"
    )
        ...
        if plot and plot.get("occupied"):
            resident = next(
                (
                    p
                    for p in participants
                    if str(p.get("id", "")).lower().strip()
                    == str(plot.get("user_id", "")).lower().strip()
                ),
                None,
            )
            plot_export_data.append(
                {
                    ...
                    "Owner Name": plot.get(
                        "user_name", resident.get("name", "") if resident else ""
                    ),
                    "Contact": plot.get(
    ...
    for act in acts:
        ...
        unique_participants = sum(
            1 for pid, counts in activity_counts.items() if counts[act_name] > 0
        )
        ...
        c3.metric(
            "Participation Rate",
            f"{(unique_participants / active * 100):.1f}%" if active > 0 else "0%",
        )
    ...
    if qr_mode == "🔍 Search Specific Resident":
        qr_search = st.text_input(
            "Search resident to generate QR code",
            placeholder="Type name or ID...",
            key="qr_search_individual",
        )
        ...
        st.write(
            f"**Total Active Residents:** {len([p for p in participants if p.get('active', True)])}"
        )
        ...
        qr_filter = st.selectbox(
            ...
            [
                ...
                "New Residents Only",
                "Regular Residents Only",
                "Without Block Number",
            ],
        ...
        if qr_filter == "New Residents Only":
            ...
            filtered_participants = [
                p for p in filtered_participants if not p.get("is_new")
            ]
        elif qr_filter == "Without Block Number":
            filtered_participants = [
                p for p in filtered_participants if not p.get("block_no")
            ]
                ...
                with cols[i % 3]:
                    with st.container():
                        ...
                        if st.session_state.get(f"show_qr_{resident_id}"):
                            _display_qr_code(p)
                            if st.button(
                                "❌ Close", key=f"close_qr_{resident_id}"
                            ):
...
def _display_qr_code(p):
    ...
    # Generate QR code as base64 to avoid CORS issues
    import requests
    import base64
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            qr_image_src = qr_api_url  # Fallback to URL
    except:
        qr_image_src = qr_api_url  # Fallback to URL
