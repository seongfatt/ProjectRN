...
def show_reports(selected_date):
    ...
    def load_data(s, e, act):
        try:
            ...
            if act != "All Activities":
                q = q.eq('source', act)
            return q.execute().data
        except Exception as ex:
            print(f"Report load error: {ex}")
            return []
    ...
    # 🔥 Ensure all session columns exist even if DB lacks them
    for col in ['session_1', 'session_2', 'session_3', 'session_4']:
    ...
    s3 = sum(1 for r in data if r.get('session_3'))
    s4 = sum(1 for r in data if r.get('session_4'))

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Records", total); c2.metric("Unique", unique)
    c3.metric("Session 1", s1); c4.metric("Session 2", s2)
    c5.metric("Session 3", s3); c6.metric("Session 4", s4)
    st.divider()

    st.subheader("Detailed Records")
    disp = df[['date', 'name', 'session_1', 'session_2', 'session_3', 'session_4', 'source', 'timestamp']].copy()
    disp.columns = ['Date', 'Name', 'S1', 'S2', 'S3', 'S4', 'Activity', 'Time']
    for col in ['S1', 'S2', 'S3', 'S4']:
        disp[col] = disp[col].map({True: 'Yes', False: 'No'})
    st.dataframe(disp, use_container_width=True, hide_index=True)
    st.divider()

    st.subheader("Participant Frequency")
    freq = df.groupby('name').agg({
        'date': 'count',
        'session_1': 'sum', 'session_2': 'sum', 'session_3': 'sum', 'session_4': 'sum'
    }).reset_index()
    freq.columns = ['Name', 'Days', 'Session1', 'Session2', 'Session3', 'Session4']
    ...
    if st.button("Export Summary"):
        summary = pd.DataFrame({'Period': [f"{start.strftime('%d %b %Y')} to {end.strftime('%d %b %Y')}"],
