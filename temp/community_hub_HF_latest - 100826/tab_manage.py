    ...
    with tab1:
        if st.session_state.user_role == 'admin':
            with st.expander("Register New Participant"):
                # 🔥 FIX: Removed st.form to match tab_volunteer format
                # (allows instant checkbox reactivity for the block consent checkbox)
                ...
                # 🔥 NEW: Block information (same format as tab_volunteer)
                ...
                if st.button("Register", type="primary", use_container_width=True, key="new_p_submit"):
                    if name:
                        ...
                        new_p = {
                            ...
                            "block_no": block_no if block_no else None,   # 🔥 NEW: save block number
                            "indemnity": indemnity,
                            "is_new": True,
                            "active": True,
                        ...
                        # Clear the fields (mimics form clear-on-submit)
            ...
            with st.expander("🗑️ Remove Participant"):
                ...
                # 🔥 NEW: Show All Active Residents List
                            ...
                            with st.container():
                                ...
                                if c3.button("Remove", key=f"remove_{p['id']}", type="secondary"):
                                    try:
                                        ...
                                        supabase.table('participants').update({'active': False}).eq('id', p['id']).execute()
                                        # 🔥 CRITICAL: Force a full data refresh immediately
                                        st.session_state.participants = load_participants()
                                        refresh_data()
                                        st.success(f"✅ {p['name']} removed successfully.")
                                        st.rerun()
        ...
        # ── TAB 2: Activities (🔥 DYNAMIC SESSIONS: add / remove up to 4) ──
    with tab2:
        MAX_SESSIONS = 4
        # 🔥 Load ALL activities (including inactive) so they can be restored
        try:
        ...
        if not acts:
            ...
            with c4:
                if st.session_state.user_role == 'admin':
                    # 🔥 Restore button for deactivated activities
                    if not a.get('active'):
                ...
                # ── EDIT MODE with dynamic sessions ──
                            ...
                            if st.form_submit_button("💾 Save Changes", type="primary"):
                                ...
                                for i in range(1, MAX_SESSIONS + 1):
                                    if i <= num:
                                        ...
                                        # 🔥 removed sessions are fully cleared
        ...
        # ── ADD NEW ACTIVITY with dynamic sessions ──
        if st.session_state.user_role == 'admin':
