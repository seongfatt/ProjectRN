...
def show_settings():
    ...
    current_token = get_setting('token_retention', '30 days')
    current_session = get_setting('session_retention', '30 days')
    current_attendance = get_setting('attendance_retention', '90 days')
    ...
    with qc1:
        days_token = int(token_ret.split()[0]) if token_ret != "Never" and token_ret != "1 year" else (365 if token_ret == "1 year" else 0)
    ...
    with qc2:
        days_sess = int(sess_ret.split()[0]) if sess_ret != "Never" and sess_ret != "1 year" else (365 if sess_ret == "1 year" else 0)
    ...
    with qc3:
        days_att = int(att_ret.split()[0]) if att_ret != "Never" and att_ret != "1 year" else (365 if att_ret == "1 year" else 0)
