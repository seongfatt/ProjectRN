...
if st.session_state.is_authenticated:
    if st.session_state.user_role == "admin":
        # 🔥 SIMPLIFIED: 9 tabs with new Dashboard + Check-In
        tabs = st.tabs([
            "📊 Dashboard",          # NEW: Power BI-style dashboard
            "📱 Check-In",           # NEW: McDonald's-style kiosk
            " Sessions",           # Session management
            "🤝 Volunteer",          # Volunteer registration
            "🔐 Volunteer Access",   # Volunteer link management
            "📊 Reports",            # Attendance reports
            "⚙️ Manage",            # Participant management
            " Garden",             # Garden plots
            " Residents",          # Resident directory
            "📋 Meeting"             # Meeting management
        ])
        with tabs[0]: show_dashboard()  # NEW dashboard
        with tabs[1]: show_checkin(selected_date)  # NEW kiosk
        with tabs[2]: show_sessions(supabase, st.session_state.user_role)
        with tabs[3]: show_volunteer()
        with tabs[4]: show_volunteer_access()
        with tabs[5]: show_reports(selected_date)
        with tabs[6]: show_manage(selected_date)
        with tabs[7]: show_garden()
        with tabs[8]: show_residents()
        with tabs[9]: show_meeting(selected_date)
        
    elif st.session_state.user_role == "chairman":
        # 🔥 CHAIRMAN: 8 tabs
        tabs = st.tabs([
            "📊 Overview",
            "📊 Dashboard",          # NEW: Dashboard for chairman
            "📱 Check-In",           # NEW: Kiosk for chairman
            "📅 Sessions",
            "🔐 Volunteer Access",
            "📊 Reports",
            " Garden",
        ...
        with tabs[1]: show_dashboard()  # NEW dashboard
        with tabs[2]: show_checkin(selected_date)  # NEW kiosk
        with tabs[3]: show_sessions(supabase, st.session_state.user_role)
        with tabs[4]: show_volunteer_access()
        with tabs[5]: show_reports(selected_date)
