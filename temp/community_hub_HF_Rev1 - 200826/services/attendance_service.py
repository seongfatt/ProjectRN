...
class AttendanceService:
    ...
    def process_checkin(pid, date, activity, s1, s2, s3=False, s4=False):
        ...
        try:
            ...
            # 🛡️ FIX: Normalize incoming ID
            pid_clean = str(pid).strip()
            
            # Get resident using a manual filter to trim invisible spaces
            res_data = supabase.table('participants').select("*").execute()
            
            resident = None
            for row in res_data.data:
                # 🛡️ FIX: Trim the database ID before comparing
                if row.get('id') and str(row['id']).strip() == pid_clean:
                    resident = row
                    break
            
            if not resident:
                return False, f"❌ Resident ID not found: {pid_clean}", None
            
            # Check existing attendance
            existing = supabase.table('attendance').select("*") \
                .eq('participant_id', resident['id']).eq('date', formatted_date).eq('source', activity).execute()
            ...
            if existing.data and len(existing.data) > 0:
                ...
                sync_session_attendance_async(resident['id'], resident['name'], activity, formatted_date,
            ...
            # 🔥 ADD THIS LINE TO SHOW THE SUCCESS MESSAGE!
            ...
            if 'duplicate key' in str(e):
                ...
                st.error(f"❌ HIDDEN ERROR FOUND: {e}")
